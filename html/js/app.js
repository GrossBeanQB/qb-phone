...
QB.Phone.Functions.SetupApplications = function(data) {
    ...
    for (var i = 1; i <= QB.Phone.Data.MaxSlots; i++) {
...
$(document).on('click', '.phone-application', function(e){
    ...
    if (AppObject.length !== 0 && CanOpenApp) {
        if (QB.Phone.Data.currentApplication == null) {
            ...
            switch (PressedApplication) {
                ...
                case "phone":
                    ...
                    $.post('https://qb-phone/ClearGeneralAlerts',JSON.stringify({app:"phone"}));
                    break;
                case "mail":
                    $.post('https://qb-phone/GetMails',JSON.stringify({}),function(mails){
                        QB.Phone.Functions.SetupMails(mails);
                    });
                    $.post('https://qb-phone/ClearGeneralAlerts',JSON.stringify({app:"mail"}));
                    break;
                case "advert":
                ...
                case "garage":
                    ...
                    break;
                case "crypto":
                    $.post('https://qb-phone/GetCryptoData',JSON.stringify({crypto:"qbit"}),function(CryptoData){
...
$(document).on('click', '.mykeys-key', function(e){
    ...
    $.post('https://qb-phone/SetHouseLocation',JSON.stringify({HouseData:KeyData}));
});
...
$(document).on('click', '.phone-home-container', function(event){
    ...
    if (QB.Phone.Data.currentApplication === null) {
        ...
        switch (QB.Phone.Data.currentApplication) {
            case "whatsapp":
                if (OpenedChatData.number !== null) {
                    setTimeout(function(){
                        $(".whatsapp-chats").css({"display":"block"}).animate({left:"0vh"},1);
                        $(".whatsapp-openedchat").animate({left:"-30vh"},1,function(){
            ...
            case "bank":
                if (CurrentTab == "invoices") {
                    setTimeout(function(){
                        ...
                        $(".bank-app-accounts").css({"display":"block","left":"0vh"});
            ...
            case "meos":
                ...
                setTimeout(function(){
                    $(".meos-recent-alert").removeClass("noodknop").css({"background-color":"#004682"});
                },400);
                break;
        }
...
QB.Phone.Animations.BottomSlideUp = function(Object,Timeout,Percentage) {
    $(Object).css({'display':'block'}).animate({bottom:Percentage+"%"},Timeout);
};

QB.Phone.Animations.BottomSlideDown = function(Object,Timeout,Percentage) {
    $(Object).css({'display':'block'}).animate({bottom:Percentage+"%"},Timeout,function(){
        $(Object).css({'display':'none'});
    });
};

QB.Phone.Animations.TopSlideDown = function(Object,Timeout,Percentage) {
    $(Object).css({'display':'block'}).animate({top:Percentage+"%"},Timeout);
};

QB.Phone.Animations.TopSlideUp = function(Object,Timeout,Percentage) {
    $(Object).css({'display':'block'}).animate({top:Percentage+"%"},Timeout,function(){
        $(Object).css({'display':'none'});
    });
};
...
QB.Phone.Notifications.Add = function(icon,title,text,color,timeout) {
    $.post('https://qb-phone/HasPhone',JSON.stringify({}),function(HasPhone){
        if (HasPhone) {
            ...
            if (color != null) {
                $(".notification-icon, .notification-title").css({"color":color});
            }
            $(".notification-title").html(title);
            $(".notification-text").html(text);
            if (QB.Phone.Notifications.Timeout) clearTimeout(QB.Phone.Notifications.Timeout);
...
QB.Screen.Notification = function(title,content,icon,timeout,color) {
    $.post('https://qb-phone/HasPhone',JSON.stringify({}),function(HasPhone){
        if (HasPhone) {
            if (color) $(".screen-notifications-container").css({"background-color":color});
            $(".screen-notification-icon").html('<i class="'+icon+'"></i>');
            $(".screen-notification-title").text(title);
            $(".screen-notification-content").text(content);
            $(".screen-notifications-container").css({'display':'block'}).animate({right:"5vh"},200);
            if (NotificationTimeout) clearTimeout(NotificationTimeout);
            NotificationTimeout = setTimeout(function(){
                $(".screen-notifications-container").animate({right:"-35vh"},200,function(){
    ...
    window.addEventListener('message',function(event){
        switch (event.data.action) {
            ...
            case "UpdateMentionedTweets":
                ...
                break;
            case "UpdateBank":
                $(".bank-app-account-balance").html("&#36; "+event.data.NewBalance).data('balance',event.data.NewBalance);
                break;
            case "UpdateChat":
            ...
            case "CancelOutgoingCall":
                $.post('https://qb-phone/HasPhone',JSON.stringify({}),function(HasPhone){
                    if (HasPhone) CancelOutgoingCall();
                });
                break;
            case "IncomingCallAlert":
                $.post('https://qb-phone/HasPhone',JSON.stringify({}),function(HasPhone){
                    if (HasPhone) IncomingCallAlert(event.data.CallData,event.data.Canceled,event.data.AnonymousCall);
                });
                break;
            case "SetupHomeCall":
                QB.Phone.Functions.SetupCurrentCall(event.data.CallData);
            ...
            case "UpdateCallTime":
                ...
                if (!QB.Phone.Data.IsOpen) {
                    if ($(".call-notifications").css("right") !== "52.1px") {
                        $(".call-notifications").css({"display":"block"}).animate({right:"5vh"});
                    }
                    ...
                    $(".call-notifications").animate({right:"-35vh"},400,function(){
                        $(".call-notifications").css({"display":"none"});
                    });
                }
                $(".phone-call-ongoing-time, .phone-currentcall-title").html("In conversation ("+timeString+")");
                break;
            case "CancelOngoingCall":
                $(".call-notifications").animate({right:"-35vh"},function(){
                    $(".call-notifications").css({"display":"none"});
                });
                QB.Phone.Animations.TopSlideUp('.phone-application-container',400,-160);
                setTimeout(function(){
